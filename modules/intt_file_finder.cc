...
void intt_file_finder::CheckDataDir() {
...
void intt_file_finder::FormatRunNumber() {
...
void intt_file_finder::FindFiles() {
    // Clear the existing file list
    m_file_list.clear();

    // Loop through all flxnumbers (0 to 5)
    for (unsigned int flx_number = 0; flx_number < 8; ++flx_number) {
        // Formulate the bash command with the wildcard pattern
        std::string bash_command = "ls " + m_data_path + "/" + m_run_type + "/*intt" + std::to_string(flx_number) + "*" + m_run_number_str + "*.evt";
        ...
        //   std::cerr << "Could Not find files for" << m_run_type << " " << m_run_number_str << " flx " << flx_number << std::endl;
            //try .prdf
            bash_command = "ls " + m_data_path + "/" + m_run_type + "/*intt" + std::to_string(flx_number) + "*" + m_run_number_str + "*.prdf";
...
void intt_file_finder::CheckRunType() {
...
std::vector<std::string> intt_file_finder::GetFiles(const unsigned int flx_number) {
    // Check if the directory has been checked and files filled
    if (flx_number > 7 || flx_number < 0) {
