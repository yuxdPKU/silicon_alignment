...
ClassImp(YMLPAnalyzer);

////////////////////////////////////////////////////////////////////////////////
/// Destructor

YMLPAnalyzer::~YMLPAnalyzer()
{
...
Int_t YMLPAnalyzer::GetNeurons(Int_t layer)
{
   if(layer==1) {
      TString fStructure = fNetwork->GetStructure();
...
TString YMLPAnalyzer::GetNeuronFormula(Int_t idx)
{
   TString fStructure = fNetwork->GetStructure();
   TString input      = TString(fStructure(0, fStructure.First(':')));
   Int_t beg = 0;
...
const char* YMLPAnalyzer::GetInputNeuronTitle(Int_t in)
{
   YNeuron* neuron=(YNeuron*)fNetwork->fFirstLayer[in];
...
const char* YMLPAnalyzer::GetOutputNeuronTitle(Int_t out)
{
   YNeuron* neuron=(YNeuron*)fNetwork->fLastLayer[out];
   return neuron ? neuron->GetName() : "NO SUCH NEURON";
}
...
void YMLPAnalyzer::CheckNetwork()
{
   TString fStructure = fNetwork->GetStructure();
...
void YMLPAnalyzer::GatherInformations()
{
   Double_t shift = 0.1;
   ...
   for(j=0; j< nEvents; j++) {
      ...
      for(k=0; k<GetNeurons(GetLayers()); k++) {
         ...
         trueVal[k] = ((YNeuron*)fNetwork->fLastLayer[k])->GetBranch();
      }
      fIOTree->Fill();
...
void YMLPAnalyzer::DrawDInput(Int_t i)
{
   char sel[64];
   snprintf(sel,64, "inNeuron==%d", i);
...
void YMLPAnalyzer::DrawNetwork(Int_t neuron, const char* signal, const char* bg)
{
   TTree* data = fNetwork->fData;
   TEventList* test = fNetwork->fTest;
...
TProfile* YMLPAnalyzer::DrawTruthDeviation(Int_t outnode /*=0*/,
                                           Option_t *option /*=""*/)
{
...
THStack* YMLPAnalyzer::DrawTruthDeviations(Option_t *option /*=""*/)
{
...
TProfile* YMLPAnalyzer::DrawTruthDeviationInOut(Int_t innode,
                                                Int_t outnode /*=0*/,
                                                Option_t *option /*=""*/)
{
   if (!fIOTree) GatherInformations();
...
THStack* YMLPAnalyzer::DrawTruthDeviationInsOut(Int_t outnode /*=0*/,
                                                Option_t *option /*=""*/)
{
   TString sName;
